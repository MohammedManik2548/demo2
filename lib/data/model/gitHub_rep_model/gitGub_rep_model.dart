...
//
//     final gitHubRepoModel = gitHubRepoModelFromJson(jsonString);

import 'dart:convert';

List<GitHubRepoModel> gitHubRepoModelFromJson(String str) => List<GitHubRepoModel>.from(json.decode(str).map((x) => GitHubRepoModel.fromJson(x)));

String gitHubRepoModelToJson(List<GitHubRepoModel> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
...
class GitHubRepoModel {
  ...
  factory GitHubRepoModel.fromJson(Map<String, dynamic> json) => GitHubRepoModel(
    id: json["id"],
    nodeId: json["node_id"],
    name: json["name"],
...
enum EventsUrl {
  HTTPS_API_GITHUB_COM_USERS_SABINONWEB_EVENTS_PRIVACY
}

final eventsUrlValues = EnumValues({
  "https://api.github.com/users/sabinonweb/events{/privacy}": EventsUrl.HTTPS_API_GITHUB_COM_USERS_SABINONWEB_EVENTS_PRIVACY
});

enum FollowingUrl {
  HTTPS_API_GITHUB_COM_USERS_SABINONWEB_FOLLOWING_OTHER_USER
}

final followingUrlValues = EnumValues({
  "https://api.github.com/users/sabinonweb/following{/other_user}": FollowingUrl.HTTPS_API_GITHUB_COM_USERS_SABINONWEB_FOLLOWING_OTHER_USER
});

enum GistsUrl {
  HTTPS_API_GITHUB_COM_USERS_SABINONWEB_GISTS_GIST_ID
}

final gistsUrlValues = EnumValues({
  "https://api.github.com/users/sabinonweb/gists{/gist_id}": GistsUrl.HTTPS_API_GITHUB_COM_USERS_SABINONWEB_GISTS_GIST_ID
});
...
final nodeIdValues = EnumValues({
  "U_kgDOB1meFw": NodeId.U_KG_DOB1_ME_FW
});

enum StarredUrl {
  HTTPS_API_GITHUB_COM_USERS_SABINONWEB_STARRED_OWNER_REPO
}

final starredUrlValues = EnumValues({
  "https://api.github.com/users/sabinonweb/starred{/owner}{/repo}": StarredUrl.HTTPS_API_GITHUB_COM_USERS_SABINONWEB_STARRED_OWNER_REPO
});
